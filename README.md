...
3. 打开管理面板 →「添加新脚本」
4. 将仓库根目录的 [`speedup.user.js`](./speedup.user.js) 全文粘贴保存（已装过的请整份覆盖升级到 **v1.4.0+**）
5. 打开 B 站视频页或 YouTube 视频页，**硬刷新**（Cmd/Ctrl+Shift+R）
6. 打开控制台应看到：`[SpeedUp] v1.4.0 active on ...`；播放器控制栏应出现倍速按钮（如 `1.0x`）
...
3. 控制台有无 `[SpeedUp] v1.4.0 active`；没有 = 脚本未注入
