...
<p align="center">
  <img src="./logo.png" alt="SpeedUp logo" width="160">
</p>

油猴脚本：为 **Bilibili** 与 **YouTube** 提供更丰富的播放倍速（预设 + 自定义），记忆倍率，并支持长按 O / P 临时变速。

当前版本：**1.8.2**。每次影响用户行为的发布都会更新油猴元数据中的 `@version`。
...
| 短按 **P** / **O** | 视频播放中且不在文本输入时，前进 / 后退 5 秒 |
...
- **YouTube**：官方设置里的倍速 API 常卡在约 2×，因此在控制栏右侧注入 `ytp-button` 风格的倍速按钮 + 菜单；长按 O/P 时该按钮文字会更新。广告播放时使用**独立的广告倍速**（默认 2×，与正常视频倍速分开）；广告结束后自动恢复你设定的正常倍速。
...
YouTube 广告倍速（与正常视频倍速独立，可在脚本顶部 `CONFIG` 修改）：

```js
youtubeAdRate: 2.0, // 广告播放时的倍速；控制栏仍显示你设定的正常倍速
```
...
- **YouTube 字幕**：脚本会同步通知播放器内部倍速，避免字幕停留过久；若倍速超过约 2×，YouTube 内部调度可能仍略有偏差。
