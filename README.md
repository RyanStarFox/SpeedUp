...
3. 打开管理面板 →「添加新脚本」
4. 将仓库根目录的 [`speedup.user.js`](./speedup.user.js) 全文粘贴保存（已装过的请整份覆盖升级到 **v1.2.0+**）
5. 打开 B 站视频页或 YouTube 视频页，**硬刷新**（Cmd/Ctrl+Shift+R）
6. 打开控制台应看到：`[SpeedUp] v1.2.0 active on ...`；播放器控制栏应出现倍速按钮（如 `1.0x`）
...
- **Bilibili**：隐藏原生「倍速」控件，在原位置插入脚本自己的倍速按钮/菜单；在页面上下文改写 `playbackRate`，防止被强制拉回 `1×`。脚本不会再修改 Shadow DOM 初始化逻辑。
...
3. 控制台有无 `[SpeedUp] v1.2.0 active`；没有 = 脚本未注入
