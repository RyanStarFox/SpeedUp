...
| 长按 **.** | 按下 ≥0.5s 后，在当前设定倍速上 ×1.5（十分位四舍五入） |
| 长按 **,** | 按下 ≥0.5s 后，在当前设定倍速上 ×0.5 |
| 松手 | 恢复为已设定的基础倍速 |
| **O** / **P** | 播放器聚焦且播放中时，永久减速 / 加速 `0.5`；长按 ≥0.5s 后每 0.2s 重复 |
| **;** / **'** | 播放器聚焦且播放中时，永久减速 / 加速 `0.1`；长按 ≥0.5s 后每 0.1s 重复 |
| **,** / **.** | 播放器聚焦且播放中时，临时 `×0.5 / ×1.5`；暂停时保留网站原生逐帧功能 |
